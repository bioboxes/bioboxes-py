...
def test_remap_args_with_single_entry():
    input    = [{"id" : 0 , "value" : reads_1, "type": "paired"}]
    expected = [{"id" : 0 , "value" : "/bbx/mount/0/reads_1", "type": "paired"}]
    assert cnf.remap_biobox_input_paths(wrap("fastq", input)) == wrap("fastq", expected)
    assert cnf.remap_biobox_input_paths(wrap("fasta", input)) == wrap("fasta", expected)
    assert cnf.remap_biobox_input_paths(wrap("reference", input)) == wrap("reference", expected)

def test_remap_args_with_single_fastq():
    input    = [{"id" : 0 , "value" : reads_1, "type": "paired"}]
    expected = [{"id" : 0 , "value" : "/bbx/mount/0/reads_1", "type": "paired"}]
    assert cnf.remap_biobox_input_paths(wrap("fastq", input)) == wrap("fastq", expected)
...
def test_remap_args_with_two_files_in_different_dir():
    ...
    output = cnf.remap_biobox_input_paths(wrap("fastq", input))
